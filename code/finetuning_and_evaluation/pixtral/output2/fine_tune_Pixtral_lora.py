...
# ---------- CLI Arguments ----------
parser = argparse.ArgumentParser(description='PEFT fine-tuning for Pixtral')
...
parser.add_argument('--model_dir', type=str, default="pixtral-finetuned")
parser.add_argument('--max_tokens', type=int, default=2048)
parser.add_argument('--cur_dir_path', type=str, default=os.getcwd())
parser.add_argument('--batch_size', type=int, default=1)
parser.add_argument('--accum', type=int, default=8)  # Increased from 4 to 8
parser.add_argument('--lr', type=float, default=2e-4)
parser.add_argument('--num_train_epochs', type=int, default=3)
parser.add_argument('--weight_decay', type=float, default=0.0001)
parser.add_argument('--permitted_max_tokens', type=int, default=29500)
...
# Pixtral does not have a pad token, set eos token as pad token
processor.tokenizer.pad_token = processor.tokenizer.eos_token
processor.tokenizer.pad_token_id = processor.tokenizer.eos_token_id

# ---------- LoRA Config ----------
# Pixtral is Mistral-based, so we target its MLP + attention projections
...
images_path = f'{script_dir}/{args.images_json}'  #os.path.join(script_dir, args.images_json)
...
if os.path.exists(log_file_path):
    ...
    with open(log_file_path, "r") as f:
        for line in f:
            if line.startswith("SKIPPED: Prompt key '"):
                # Extract the key from lines like: "SKIPPED: Prompt key 'some_key' - ..."
                key = line.split("'")[1]
    ...
    for i, p in tqdm(enumerate(prompts), total=len(prompts), desc="Building dataset"):
        ...
        # If this key was previously skipped, skip it again
        ...
        # Add to dataset without token counting, include key
...
class PixtralCollator:
    ...
    def __call__(self, batch):
        ...
        ## Pixtral does not have pad token, but we do not need it since we train with batch size 1, every sequence ends with eos

        labels[labels == self.processor.tokenizer.pad_token_id] = -100
        if self.image_token_id is not None:
            labels[labels == self.image_token_id] = -100
        
        # Mask everything before [/INST] to only train on assistant response
        inst_end_token = "[/INST]"
        # Get the token ID for [/INST] if it exists
        inst_token_id = self.processor.tokenizer.convert_tokens_to_ids("[/INST]")
        
        for i in range(len(texts)):
            try:
                # Find where [/INST] token appears in the encoded sequence
                input_ids_list = enc["input_ids"][i].tolist()
                
                # Search for [/INST] token in the sequence
                if inst_token_id in input_ids_list:
                    # Find the position of [/INST] token
                    inst_position = input_ids_list.index(inst_token_id)
                    # Mask everything up to and including [/INST]
                    labels[i, :inst_position + 1] = -100
                else:
                    # If [/INST] not found at all, mask everything (safety fallback)
                    # This should rarely happen as [/INST] is part of the instruction format
        ...
        # Count and print token statistics
        input_ids = enc["input_ids"]
        attention_mask = enc["attention_mask"]
        
        # Total input tokens (excluding padding)
        total_input_tokens = attention_mask.sum().item()
        
        # Trainable output tokens (labels that are not -100)
        trainable_tokens = (labels != -100).sum().item()
        
        # Print token stats for this batch (only on rank 0 to avoid duplicates)
...
# ---------- Training Arguments ----------
deepspeed_cfg = os.path.join(args.cur_dir_path, "deepspeed_pixtral.json")
with open(deepspeed_cfg, "w") as f:
    json.dump({
...
training_args = TrainingArguments(
    ...
    deepspeed=deepspeed_cfg,  # Add DeepSpeed config
    local_rank=int(os.environ.get("LOCAL_RANK", -1)),  # Required for DeepSpeed
