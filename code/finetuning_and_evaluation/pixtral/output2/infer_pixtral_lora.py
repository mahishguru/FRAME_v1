...
# ---------- CLI Arguments ----------
parser = argparse.ArgumentParser(description='Inference with LoRA fine-tuned Pixtral model')
parser.add_argument('--model_path', type=str, required=True,
                    help='Path to fine-tuned model checkpoint (e.g., pixtral-12b/checkpoint-final)')
...
def run_inference(args):
    ...
    print("=" * 80)
    print("Pixtral LoRA Fine-tuned Model Inference")
    print("=" * 80)
    ...
    for prompt_data in tqdm(remaining, desc="Generating", unit="sample"):
        ...
        # Extract ONLY the user prompt (everything before and including [/INST])
        if "[/INST]" in full_prompt:
