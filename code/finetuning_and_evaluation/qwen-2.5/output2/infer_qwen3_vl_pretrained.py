...
# ---------- CLI Arguments ----------
parser = argparse.ArgumentParser(description='Inference with PRETRAINED (base) Qwen3-VL model - NO fine-tuning')
...
def load_model_and_processor(model_name, device, use_flash_attention):
    """Load the PRETRAINED (base) model and processor - NO LoRA adapter."""
    print(f"Loading processor from: {model_name}")
    processor = AutoProcessor.from_pretrained(model_name)
    
    print(f"Loading PRETRAINED model: {model_name}")
    print(f"NOTE: This is the BASE model WITHOUT fine-tuning")
    print(f"Flash Attention: {'Enabled' if use_flash_attention else 'Disabled'}")
    
    # Load the base model directly (NO LoRA adapter)
...
def main():
    print("\n" + "=" * 80)
    print("Qwen3-VL-8B-Instruct PRETRAINED Model Inference (NO fine-tuning)")
    print("=" * 80 + "\n")
    
    # Load test data
    prompts, images_data = load_test_data(args.test_prompts_jsonl, args.test_images_json)
    
    # Load model and processor (PRETRAINED - no LoRA)
    ...
    for i, prompt_data in enumerate(tqdm(filtered_prompts, desc="Generating")):
        ...
        try:
            ...
            output_data = {
                ...
                "model_type": "PRETRAINED (base model, no fine-tuning)",
    ...
    print("\n" + "=" * 80)
    print("Inference Complete! (PRETRAINED model)")
