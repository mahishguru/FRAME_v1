...
# ---------- CLI Arguments ----------
parser = argparse.ArgumentParser(description='PEFT fine-tuning for Qwen3-VL-8B-Instruct')
...
class Qwen3VLCollator:
    ...
    def __call__(self, batch):
        ...
        # Collect all texts and images from batch
        texts = []
        all_images = []
        
        for ex in batch:
            # Get pre-formatted text (already has <|im_start|> tags)
            texts.append(ex['prompt'])
            
            # Load images for this example
            paths = ex['image_path']
            imgs = []
            for p in paths:
                try:
        ...
        # Process text and images directly (bypass apply_chat_template)
        # The processor can handle pre-formatted text with <|vision_start|><|image_pad|> tokens
        ...
        # Create labels - mask padding and everything before assistant response
        labels = input_ids.clone()
        labels[labels == self.processor.tokenizer.pad_token_id] = -100

        # Mask everything before assistant response to only train on assistant output
        # Following Pixtral's exact approach
        # Qwen3-VL uses "assistant" as the role marker
        assistant_token_id = self.processor.tokenizer.convert_tokens_to_ids("assistant")
        
        for i in range(len(texts)):
            try:
                # Find where assistant token appears in the encoded sequence
                input_ids_list = input_ids[i].tolist()
                
                # Search for assistant token in the sequence
                if assistant_token_id in input_ids_list:
                    ...
                    # If assistant token not found, mask entire sequence (safety fallback)
...
# ---------- Training Arguments ----------
deepspeed_config_path = os.path.join(args.cur_dir_path, "deepspeed_qwen3vl.json")
...
# ---------- Train ----------
print("\n" + "="*80)
print("Starting Qwen3-VL-8B-Instruct Fine-Tuning")
print("="*80 + "\n")
