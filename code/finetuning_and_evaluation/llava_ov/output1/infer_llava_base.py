...
# ---------- CLI Arguments ----------
parser = argparse.ArgumentParser(description='Inference with BASE LLaVA-OV model (no fine-tuning)')
...
def load_base_model_and_processor(base_model, device):
    """Load the BASE model (no LoRA) and processor."""
    print(f"Loading processor from: {base_model}")
    processor = AutoProcessor.from_pretrained(base_model)
    
    print(f"Loading BASE model from: {base_model}")
    print("⚠️  Note: This is the PRE-TRAINED model (NOT fine-tuned)")
...
def run_inference(args):
    ...
    print("=" * 80)
    print("LLaVA-OV BASE Model Inference (No Fine-tuning)")
    print("=" * 80)
    ...
    # Step 4: NOW load the base model (NOT fine-tuned)
